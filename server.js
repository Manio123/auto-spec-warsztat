...
app.use(session({ secret: 'auto-spec-ultra-secret', resave: false, saveUninitialized: true }));
            ...
            <div class="product-card">
                ...
                <div class="product-details">
                    ...
                    <div style="display:flex; gap:10px; align-items:center; margin-bottom:15px;">
                        <label>Ilość:</label>
                        <input type="number" id="qty-${p.id}" value="1" min="1" style="width:60px; margin:0;">
                    </div>
        ...
        db.all("SELECT * FROM products", (err, prods) => {
            const rList = reqs.map(r => `
                <div class="req-item" style="display:flex; justify-content:space-between; align-items:center;">
                    <div><b>${r.car}</b> (${r.client})<br><small>Tel: ${r.phone} | Usługa: ${r.service_type}</small></div>
                    <a href="/admin/del-r/${r.id}" class="btn btn-danger" style="padding:5px 10px; font-size:12px; text-decoration:none;">Usuń</a>
                </div>
            `).join('');
            const pList = prods.map(p => `<li style="margin-bottom:8px;">${p.name} - ${p.price} zł <a href="/admin/del-p/${p.id}" style="color:red; margin-left:10px;">[Usuń]</a></li>`).join('');
            res.send(`<html><head><link rel="stylesheet" href="style.css"></head><body class="container"><h1>Panel Manager</h1><a href="/logout">Wyloguj</a> | <a href="/">Serwis</a><div style="display:flex;gap:20px;margin-top:20px;"><div style="flex:1"><h2>Zlecenia</h2>${rList || 'Brak'}</div><div style="flex:1" class="card"><h2>Dodaj Część</h2><form action="/admin/add" method="POST"><input name="n" placeholder="Nazwa" required><input name="pr" type="number" step="0.01" placeholder="Cena" required><input name="img" placeholder="Link do zdjęcia"><textarea name="d" placeholder="Opis"></textarea><button class="btn">Dodaj</button></form><ul style="margin-top:20px;">${pList}</ul></div></div></body></html>`);
        });
    });
});
