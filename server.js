...
app.use(session({ secret: 'auto-spec-2026-secure-key', resave: false, saveUninitialized: true }));
...
app.get('/sklep', (req, res) => {
    db.all("SELECT * FROM products", (err, rows) => {
        ...
        if (!rows || rows.length === 0) {
            cardsHtml = '<div style="grid-column: 1/-1; text-align:center; padding:40px; color:#666;"><h3>Obecnie brak części w ofercie.</h3><p>Zapraszamy wkrótce!</p></div>';
        } else {
            cardsHtml = rows.map(p => `
                <div class="product-card">
                    ...
                    <div class="product-details">
                        ...
                        <p style="font-size:0.85em; color:#666; margin-bottom:15px; min-height:40px;">${p.desc || ''}</p>
                        <div style="display:flex; gap:10px; align-items:center; margin-bottom:15px;">
                            <label>Ilość:</label>
                            <input type="number" id="qty-${p.id}" value="1" min="1" style="width:60px; padding:5px; margin:0;">
                        </div>
...
app.get('/admin', (req, res) => {
    ...
    db.all("SELECT * FROM requests ORDER BY id DESC", (err, reqs) => {
        db.all("SELECT * FROM products", (err, prods) => {
            const rList = reqs.map(r => `<div class="req-item" style="display:flex; justify-content:space-between; align-items:center; padding:10px; border-bottom:1px solid #ddd;"><div><b>${r.car}</b> - ${r.client}<br><small>${r.phone} | ${r.service_type}</small></div><a href="/admin/del-r/${r.id}" class="btn" style="width:auto; background:#e74c3c; padding:5px 10px; font-size:12px; text-decoration:none; color:white;">Usuń</a></div>`).join('');
            const pList = prods.map(p => `<li style="padding:5px 0; border-bottom:1px solid #eee;">${p.name} - ${p.price} zł <a href="/admin/del-p/${p.id}" style="color:red; float:right; text-decoration:none;">[Usuń]</a></li>`).join('');
            res.send(`<html><head><link rel="stylesheet" href="style.css"></head><body class="container"><h1>Panel Manager</h1><div style="margin-bottom:20px;"><a href="/logout">Wyloguj</a> | <a href="/">Pokaż stronę</a></div><div style="display:flex;gap:30px;"><div style="flex:1"><h2>Zlecenia Serwisowe</h2>${rList || 'Brak zgłoszeń'}</div><div style="flex:1" class="card"><h2>Dodaj do Sklepu</h2><form action="/admin/add" method="POST"><input name="n" placeholder="Nazwa części" required><input name="pr" type="number" step="0.01" placeholder="Cena (np. 150.00)" required><input name="img" placeholder="Link do zdjęcia (URL)"><textarea name="d" placeholder="Opis"></textarea><button class="btn">Dodaj Produkt</button></form><ul style="margin-top:20px; list-style:none; padding:0;">${pList}</ul></div></div></body></html>`);
        });
    });
});
